...
def main(db=None):
    ...
    date_group = parser.add_argument_group(
        description="bestimmt den zu bearbeitenden Tag; falls nichts gesetzt, nutze heute:")
    date_group.add_argument('--date', type=str, default=False,
                            help="zu bearbeitender Tag im ISO Format (z.B. 1970-12-31)")
    date_group.add_argument(
        '-d', '--day', type=int, default=False, help="bestimmt den zu bearbeitenden Tag")
    date_group.add_argument('-m', '--month', type=int, default=False,
                            help="bestimmt den zu bearbeitenden Monat")
    date_group.add_argument('-y', '--year', type=int,
                            default=False, help="bestimmt das zu bearbeitende Jahr")

    day_group = parser.add_argument_group(
        description="definiert den Arbeitstag:")
    day_group.add_argument('-s', '--start', type=str, nargs='?', default=False,
                           help="setzt Arbetisbeginn, falls ohne Argument: nutze jetzt")
    day_group.add_argument('-e', '--end', type=str, nargs='?', default=False,
                           help="setzt Arbeitsende, falls ohne Argument: nutze jetzt")

    day_group.add_argument('-p', '--pause', type=int, nargs='?',
                           default=False, help="Pausenzeit in Minuten")

    day_group.add_argument('-c', '--comment', type=str, nargs='*',
                           default=None, help="optionaler Kommentar zum Eintrag")

    day_group.add_argument('--multi_day', type=str, default=False,
                           help="ermöglicht mehrere Einträge pro Tag")

    parser.add_argument('-w', '--work_time', type=str,
                        default='8:00', help="Arbeitspensum pro Tag; Format H:MM")
    ...
    parser.add_argument('--db_path', type=str, default=os.path.dirname(__file__) +
                        "/data/", help="Ort, an dem die In- und Output Files liegen")
    ...
    parser.add_argument('--azk', action='store_true',
                        default=None, help="nur Arbeitszeitkonto")
    ...
    round_up = datetime.datetime(
        year, month, day, hours, minutes, 00) + datetime.timedelta(minutes=args.round)
                                ...
                                if not args.waz:
                                    if not args.az and not args.feierabend:
                                        print("{:3}   {:5} {:5} {:^5} {:>4} {:>14}  {}".format(
                                            ...
                                            colored(d_data["Tagessaldo"] if "Tagessaldo" in d_data else "", "red" if (
                                                d_data["Tagessaldo"][0] if "Tagessaldo" in d_data else "") == "-" else "green"),
    ...
    if args.pause is not False:
        if args.pause is None:
            ...
            while not i:
                ...
                print(" {:02}:{:02}".format(int((datetime.datetime.now() - start).total_seconds()/60),
                      int((datetime.datetime.now() - start).total_seconds()) % 60), end="\r")
            if "pause" in this_day:
                this_day["pause"] += int((datetime.datetime.now() -
...
def calculate_saldos(db, work_time="8:00"):
    ...
    today_year, today_month, today_day = (
        int(t) for t in str(datetime.datetime.now().date()).split('-'))
        ...
        for m, month in year.items():
            ...
            for w, week in month.items():
                ...
                if dow >= 5 or [today_day, today_month, today_year] == [d, m, y]:
                    week["Wochenstunden"] = "{:02d}:{:02d}".format(int((week_total.days*24)+(
                        week_total.seconds/60/60)), int(((week_total.days*24)+(week_total.seconds/60/60)) % 1*60+0.5))
            ...
            try:  # single-entry day
                if "end" in day:
                    ...
                    if "pause" in day:
                        ...
                        h_incl_break_col.append(format_timedelta(
                            day_hours + datetime.timedelta(minutes=day["pause"])))
                    else:
                        break_col.append("")
